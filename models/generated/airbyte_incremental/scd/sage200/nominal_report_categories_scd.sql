{{ config(
    ...
    post_hook = ["
                    ...
                    {% endif %}
                    ","delete from _airbyte_sage200_etl_stg_pte.nominal_report_categories_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from _airbyte_sage200_etl_stg_pte.nominal_report_categories_stg)"],
    tags = [ "top-level" ]
) }}
...
new_data as (
    ...
    -- nominal_report_categories from {{ source('sage200_etl_pte', '_airbyte_raw_nominal_report_categories') }}
    where 1 = 1
