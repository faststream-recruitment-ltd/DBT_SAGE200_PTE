{{ config(
    ...
    post_hook = ["
                    ...
                    {% endif %}
                    ","delete from _airbyte_sage200_etl_stg_pte.departments_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from _airbyte_sage200_etl_stg_pte.departments_stg)"],
    tags = [ "top-level" ]
) }}
...
new_data as (
    ...
    -- departments from {{ source('sage200_etl_pte', '_airbyte_raw_departments') }}
    where 1 = 1
